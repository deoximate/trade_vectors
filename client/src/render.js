...
console.info('APP VERSION - 21.12.2023');
...
const PATH = {
  fonts: "assets/fonts/",
  models: "https://dev.hotburo.com/agilepitch/assets/models/",
  textures: "https://dev.hotburo.com/agilepitch/assets/textures/",
  shaders: "https://dev.hotburo.com/agilepitch/assets/shaders/",
}*/
...
function start() {
  ...
  //? LINES
  {
    ...
    materials.lines.transparent = true;
    //materials.lines.side = THREE.DoubleSide;

    let g = new THREE.InstancedBufferGeometry().copy(lineGeometry);
    g.instanceCount = 20 * 20;
    meshes.lines = new THREE.Mesh(g, materials.lines);
...
function update(renderData) {
  ...
  if (isAssetsLoad) {
    ...
    if (moveVec.distanceTo(mousePosPlane) < 0.001) {
      moveVec.x = mousePosPlane.x;
      moveVec.y = mousePosPlane.y;
    }
